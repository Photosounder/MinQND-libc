...
* `log2()` which works by taking the IEEE-754 of the input which directly gives part of the result and applying a polynomial to the mantissa.
...
* `asin()` which "maps" the input (the polynomials approximate stem:[arcsin(2x-x^2])) then does the upper end using a simple polynomial and ironically ends up being more accurate near 1.0 than the default `asin()` (the latter ends like a staircase whereas mine is smooth) and the lower end using a rather short polynomial that is refined using two simple Newton-Raphson steps, meaning the accuracy of my `asin()` is somewhat impacted by the inaccuracy of my `sin()`.
* `erf()` which uses an actually fairly common stem:[1 - polynomial^-8] approach.
